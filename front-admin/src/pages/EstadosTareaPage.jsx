...
export default function EstadosTareaPage() {
  ...
  const api = useMemo(() => ({
    list: () => httpJson(`${baseUrl}/api/estados-tarea`),
    get: (id) => httpJson(`${baseUrl}/api/estados-tarea/${id}`),
    create: (payload) => httpJson(`${baseUrl}/api/estados-tarea`, { method: 'POST', body: JSON.stringify(payload) }),
    updateByNombre: (nombre, payload) => httpJson(`${baseUrl}/api/estados-tarea/nombre/${encodeURIComponent(nombre)}`, { method: 'PUT', body: JSON.stringify(payload) }),
    removeByNombre: (nombre) => httpJson(`${baseUrl}/api/estados-tarea/nombre/${encodeURIComponent(nombre)}`, { method: 'DELETE' }),
    buscar: (nombre) => httpJson(`${baseUrl}/api/estados-tarea/buscar?nombre=${encodeURIComponent(nombre)}`),
  }), [baseUrl]);
  ...
  return (
    <div className="card">
      <div className="headerRow">
        <h1 className="h1">Estados de Tarea</h1>
